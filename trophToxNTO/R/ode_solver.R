...
ode.broma.tritrophic = function(Time, State, Pars) { # The model is a function
  with(as.list(c(State, Pars)), { # You need State and Pars in the next lines
    ...
    dC.soil = -k.0*C.soil - intake.V(C.soil,max.intake.C,D50.intake.C) #Sage et al. 2008 2007
    #dC.soil = 0
    ...
    dC.M = eta.M * B.V/(B.M) * (a.M*D.V+a.Md*D.Vd)/(1+h.M*(a.M*D.V+a.Md*D.Vd))*C.V -
      C.M*(k.out.M + e.M*(a.M*D.V+a.Md*D.Vd)/(1+h.M*(a.M*D.V+a.Md*D.Vd)))

    dC.F = eta.F * B.V/(B.F) * (a.F*D.V+a.Fd*D.Vd+a.FM*D.M)^2/(1+h.F*(a.F*D.V+a.Fd*D.Vd+a.FM*D.M)^2)*
      ((a.F*D.V+a.Fd*D.Vd)*C.V+a.FM*D.M*C.M)/(a.F*D.V+a.Fd*D.Vd+a.FM*D.M) - C.F*(k.out.F + r.F*(1-D.F/K.F))
    ...
    ## mustelid dynamics
    dM = e.M*(a.M*D.V+a.Md*D.Vd)/(1+h.M*(a.M*D.V+a.Md*D.Vd))*D.M- #numerical response
      m.M*D.M - # natural mortality
      dose.resp.mort(C.M,LD.50.M,H.M,days.to.die.M)*D.M - #broma poisoning
    ...
    dF = r.F*D.F*( 1 - D.F/K.F) - # logistic growth
      dose.resp.mort(C.F,LD.50.F,H.F,days.to.die.F)*D.F # broma poisoning

    ## lines afterward are necessary to kill population if under a certain number. Note that the number should be 2: 1 male + 1 femal

    return(list(c(dC.soil,dC.V, dC.M,dC.F, dV, dVd, dM, dF))) # return
  })
}
...
pars  <- function(
  ...
  e.M = 0.0025, # 0.004 Energy conversion utilisé par Wang, Nagy, Gilg, Kuang, 2009
  # Mean biomass of one individual populations - grams
  B.V = 80 , ## voles
  B.M = 300 , ## mustelid : 200-600gr weasel ;
  B.F = 5800, ## fox
  #
  ...
  # LD.50 - NOUS AVONS CONSTRUIT LE MOD7LE AVEC LES CONCENTRATIONS, pas avec les quantités totales !
  LD.50.V = 2, days.to.die.V = 6, # on est plutôt sur du 2-3 voir 6 ppm !!!
  LD.50.M = 2.1, days.to.die.M = 5,
  LD.50.F = 0.5, days.to.die.F = 5, # LD.50.F or 0.15 et 5

  H.V=4, H.M=4, H.F=2,
  # Dynamic of contaminant
  eta.M = 0.5, eta.F = 0.5, ## Absorbtion rate de 69% à 100% de 4 à 24h (voir thèse Manon Jacquot)
  k.out.V = 0.4, # voir Sage et al. 2008
  k.out.M = 0.6, k.out.F = 0.6, ## excretion rate
...
init  <- function(n.simu,C.soil=0, C.V=0, C.M=0, C.F=0,  D.V=100, D.Vd=0, D.M=0.05, D.F=0.025){
  n=n.simu
  ...
  return(c(C.soil_ = C.soil_ , C.V_=C.V_, C.M_=C.M_, C.F_=C.F_,
           D.V_ =D.V_, D.Vd_=D.Vd_,  D.M_=D.M_, D.F_ = D.F_ ))
}
