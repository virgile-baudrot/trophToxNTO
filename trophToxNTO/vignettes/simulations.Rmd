...
library(tidyr)
library(ggplot2)
# setwd("~/Documents/PACKAGES/trophToxNTO/trophToxNTO/d")
...
# PLOT POPULATION REPRODUCTION RATE

```{r reproRatePop}
library(zoo)
x = as.data.frame(model.output_500.20)

# interpolate for deltaStep
deltaStep = 30 #days
interpTime = seq(0,365*50, by = deltaStep)
interpTimeUnique = interpTime %in% x$time
df = data.frame(time = c(x$time, interpTime),
                density = c(x[, "C.V_"], rep(NA, length(interpTime))))
df = dplyr::arrange(df, time)

interpDensity = zoo::na.approx(df$density, df$time)

# uniqueness of time step
unique(sapply(1:(length(x$time)-1), function(i) x$time[i+1]-x$time[i]))

deltaSteps = sapply(1:(length(x$time)-1), function(i) x$time[i+1]-x$time[1])

rollGrowth = lapply(1:length(deltaSteps), function(j){
  tmpGrowth = sapply(1:(length(x$time)-j), function(i){ (x$C.V_[i+j]-x$C.V_[i])/deltaSteps[j]
  data.frame( growth = tmpGrowth,
              
    })
  })


rollGrowth = do.call("c", rollGrowth)
rollGrowthTime =  
rollGrowthReplicate
```
